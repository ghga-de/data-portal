...
```bash
dev_launcher data.staging.ghga.dev
```

In this case, a proxy configuration will be used that proxies all API endpoints to the staging environment, while the application itself is still served by the development server. You can also set the staging backend via the environment variable `data_portal_backend`.
